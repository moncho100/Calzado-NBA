    ...
    if("CARRITO" in localStorage){
        ...
        if(arrayLiterales.length > 0){
            for (const literal of arrayLiterales) {
                carrito.push(new Producto(literal.id, literal.nombre, literal.precio, literal.jugador, literal.cantidad, literal.imagen));
            }
            //console.log(carrito);
            carritoUI(carrito);
        }
    ...
    $.get('data/producto.json',function(datos, estado){
        ...
        if(estado == 'success'){
            for (const literal of datos) {
                productos.push(new Producto(literal.id, literal.nombre, literal.precio, literal.jugador, literal.cantidad, literal.imagen));
            }
        }
...
$('#filtroCategorias').change(function (e) { 
    ...
    $('#productosContenedor').fadeOut(600,function(){
        ...
        if(value == 'TODOS'){
            ...
            const filtrados = productos.filter(producto => producto.jugador == value);
...
$("#busquedaProducto").keyup(function (e) { 
    ...
    if(criterio != ""){
        const encontrados = productos.filter(p =>       p.nombre.includes(criterio.toUpperCase()) 
                                                    || p.jugador.includes(criterio.toUpperCase()));
